"""Tests for resolve_state_dir, EIDOLON_HOME override, source-repo refusal,
...
def test_eidolon_home_env_var_is_honored(tmp_path, monkeypatch):
    """resolve_state_dir honors EIDOLON_HOME unconditionally."""
    target = tmp_path / "custom-state"
    monkeypatch.setenv("EIDOLON_HOME", str(target))

    resolved = state.resolve_state_dir()
